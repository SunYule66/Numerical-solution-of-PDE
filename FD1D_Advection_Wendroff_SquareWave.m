function FD1D_Advection_Wendroff_SquareWave
...
for t = t_start : dt : t_final-dt
    ...
    for i = 2 : nx - 1
        uh_new(i) = uh_old(i) - 1/2*(dt/dx)*( uh_old(i+1) - uh_old(i-1) ) + (dt/dx)*(dt/dx)/2 * ( uh_old(i+1) - 2*uh_old(i) + uh_old(i-1) );
    end
    
    % treat periodic boundary conditions
    uh_new(nx) = uh_old(nx) - 1/2*(dt/dx)*( uh_old(1) - uh_old(nx-1) ) + (dt/dx)*(dt/dx)/2 * ( uh_old(1) - 2*uh_old(nx) + uh_old(nx-1) );
    uh_new(1) = uh_old(1) - 1/2*(dt/dx)*( uh_old(2) - uh_old(nx) ) + (dt/dx)*(dt/dx)/2 * ( uh_old(2) - 2*uh_old(1) + uh_old(nx) );
...
title('$\textnormal{Wendroff Scheme}$','interpreter','latex')
hold off
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
